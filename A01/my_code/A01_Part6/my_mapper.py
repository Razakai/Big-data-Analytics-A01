...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for line in my_input_stream:
        ...
        if (processedLine[11] == my_mapper_input_parameters[0]):
            if res == "":
                res = f"{processedLine[0]} @ {processedLine[1]} @ {processedLine[4]} @ {processedLine[8]}"
            else:
                res = f"{res} @ {processedLine[0]} @ {processedLine[1]} @ {processedLine[4]} @ {processedLine[8]}"
    if res != "":
        my_output_stream.write('universal' + "\t" + '(' + res + ')' + '\n')
